...
function clone(src) {
    //null 是个object, 但如果真当把当成object就。。。
    //静态 number, string , boolean, undefined...  直接返回
    //动态 array, object ... 需要递归
    // Date , Regex => object
    var result;
    if (src === null || typeof(src) != "object"){
		result = src;
    }
    else if (src.constructor == Array || src.constructor == Object){ //递归
...
function bfs(g, v0, fn) {
	...
	while (q.length > 0) {
		...
		fn(v);
		for (var i = 0; i < g.length; i++) {
			if (g[v].indexOf(i) >= 0 && visited.indexOf(i) < 0) {
				q.push(i);
				visited.push(i);
			}
		}
